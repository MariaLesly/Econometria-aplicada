...
global direct "G:\Mi unidad\PUCP\Diplomado de Especialización en Ciencia de Datos para CCSS y Gestión Pública\Repaso de Econometría\Trabajo final"
cd "$direct"
...
import delimited using "Enaho01-2023-200.csv", clear stringcols(_all) encoding("latin1")
...
import delimited using "Enaho01a-2023-300.csv", clear stringcols(_all) encoding("latin1")
...
import delimited using "Enaho01a-2023-500.csv", clear stringcols(_all) encoding("latin1")
...
drop _merge
drop codperso
save "temp_jefe_2023.dta", replace
...
keep conglome vivienda hogar codperso p207 p208a
save "dta_miembros_2023.dta", replace
...
merge 1:1 conglome vivienda hogar codperso using "dta_miembros_2023.dta"
keep if _merge == 3
drop _merge
...
save "base_2023_persona.dta", replace

* 1.3 Merge de las bases a nivel jefe de hogar, niños y hogar
*--------------------------------------------------------------------------

* 1.3.1 Información a nivel de hogar (Sumaria-2022)
import delimited using "Sumaria-2023.csv", clear stringcols(_all) encoding("latin1")
...
save "temp_sumaria_2023.dta", replace

* 1.3.2 Merge de las bases de datos creadas anteriormente
use "base_2023_persona.dta", clear
merge m:1 conglome vivienda hogar using "temp_sumaria_2023.dta" // merge con la base de hogar
keep if _merge == 3
drop _merge
merge m:1 conglome vivienda hogar using "temp_jefe_2023.dta" // merge con la base de jefe de hogar
keep if _merge == 3
drop _merge
...
save "base_2023_completa.dta", replace
...
use "base_2023_completa.dta", clear
...
estout modelo_1 modelo_2, cells(b(star fmt(3)) se(par fmt(2))) ///
    ...
    title("Tabla 2: Análisis de Asociación (OLS 2023)") ///
